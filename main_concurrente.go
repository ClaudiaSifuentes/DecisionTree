...
func bestSplit(data []DataPoint) (int, float64) {
	...
	for featureIndex := 0; featureIndex < numFeatures; featureIndex++ {
		...
		go func(f int) {
			...
			for i, d := range data {
				if i%100000 == 0 {
					fmt.Printf("[Goroutine Feature %d] Umbral %d de %d\n", f, i, len(data))
				}

				threshold := d.Features[f]
				left, right := split(data, f, threshold)
	...
	return best.FeatureIndex, best.Threshold
}
...
func main() {
	data, err := loadCSV("dataset/diabetes_big.csv")
